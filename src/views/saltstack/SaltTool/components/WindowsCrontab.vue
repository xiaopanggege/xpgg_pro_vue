<template>
  <div class="salttool-windowscrontab-container">
    <el-row :gutter="20">
        <el-col :span="12" class="overflow_set">
          <el-form :model="exeData" status-icon :rules="rules"  ref="ruleForm" label-width="120px" label-position="right" >
            <el-form-item v-show="exeData.client!='runner'" label="执行对象类型：" prop="tgt_type">
              <el-row>
            ...
            <el-form-item label="执行对象：" prop="minion_list" v-show="exeData.tgt_type=='minion_list'&&exeData.client!='runner'">
              <el-transfer
                ref="dataTransfer"
                filterable
            ...
            <!-- 执行对象输入框 -->
            <el-form-item  label='执行对象：' v-show="tgt_type_lable[exeData.tgt_type]&&exeData.client!='runner'"  prop="tgt">
              <el-input
  ...
  watch: {
    'exeData.tgt_type': {
      handler: function (val, oldVal) {
        if(this.minionid_list.length === 0 && val === 'minion_list'){
          this.getIdList();
        }
      }
    },
  ...
  methods: {
    ...
    getIdList(){
      getMinionIdList().then(response =>{
          ...
          this.minionid_list.push(minion_id);
          });
      })
    },
    // 获取命令帮助
...
.salttool-windowscrontab-container{
  .overflow_set{
