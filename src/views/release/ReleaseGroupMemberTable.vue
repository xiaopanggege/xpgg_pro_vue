  ...
  <div class="app-container" v-loading="allLoading">
    <div class="filter-container">
      ...
      <el-row class="saltminion_global">
        ...
        <el-button class="filter-item" type="primary" size="small" @click="getList">刷新</el-button>
    ...
    <el-table
      ...
      <el-table-column label="应用名称" prop="app_name" sortable="custom"  align="center" width="140">
        <template slot-scope="scope">
          <span>{{ scope.row.app_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="Minion_id" prop="minion_list" sortable="custom" align="center" width="150">
        <template slot-scope="scope">
    ...
    <el-dialog class="AppForm"  :title="dialogAppFormTitle" width="1220px" :visible.sync="dialogAppFormVisible" :close-on-click-modal="false" >
      
      <div class="overflow_set">
        <el-form ref="appDataForm"  :model="appData" label-position="right" label-width="130px" style="width: 1175px; margin:0;">
          <el-row :gutter="10">
            <el-col :span="12">
    ...
    <!-- 日志显示 -->
    <el-dialog class="logDialog" :visible.sync="dialogLogVisible" :title="logTitle + ' 日志'" width="800px">
      <!-- <el-input class="saltcmd-info-textarea" type="textarea" :autosize="{ minRows: 8, maxRows: 25}" readonly v-model="logTemp.log_content"></el-input> -->
      <el-row style="padding-bottom:10px;">
        <el-col :span="8" style="text-align:center;"><span style="font-weight:bold;">发布时间：</span><span>{{logTemp.create_time | parseTime('{y}-{m}-{d} {h}:{i}')}}</span></el-col>
        <el-col :span="8" style="text-align:center;"><span style="font-weight:bold;">发布结果：</span><span>{{logTemp.release_result}}</span></el-col>
        <el-col :span="8" style="text-align:center;"><span style="font-weight:bold;">操作人：</span><span>{{logTemp.username}}</span></el-col>
      </el-row>
      <el-row>
...
import {  releaseOpt, releaseLog, releaseGroupMemberGetList } from '@/api/release'
...
export default {
  ...
  created() {
    const group_id = this.$route.params && this.$route.params.id
    this.app_group_name = this.$route.params && this.$route.params.app_group_name
    this.listQuery.group_id = group_id
    // 官方路由tag必须加下面不然有bug
  ...
  methods: {
    ...
    getList() {
      ...
      releaseGroupMemberGetList(this.listQuery).then(response => {
        ...
        console.log('获取应用列表' + error);
      })
    },
    // tag栏设置
    setTagsViewTitle() {
      const title = '组成员发布'
      const route = Object.assign({}, this.tempRoute, { title: `${this.app_group_name}-${title}` })
    ...
    getLogList() {
      releaseLog(this.logListQuery).then(response => {
        ...
        if(Object.prototype.toString.call(this.logTemp) === '[object Object]'){
          ...
          if(Object.prototype.toString.call(eval(this.logTemp['log_content'])) === '[object Array]'){
            this.logTemp.log = '';
            for (let content of eval(this.logTemp['log_content'])){
